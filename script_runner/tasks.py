...
def download_munger_async(munger_builder_id=1):
    ...
    file_path = os.path.join(settings.MEDIA_ROOT, 'user_munger_scripts', '{0}.py'.format(mb.script_file_name))

    with open(file_path, 'r') as mf:
        response = HttpResponse(mf, content_type='application/octet-stream')
        response['Content-Disposition'] = 'filename={0}.py'.format(mb.script_file_name)
        print(response)
        return response
